...
- An account repo holds one directory per container under `gtm/containers/<slug>/`, named by a human slug derived from the container name (2026-09-24; the public id lives in `container.json` and the config file maps slug to public id): `spec.json` (canonical, the apply-able part, the authority for what it declares), `snapshot.json` (everything the API exposes, an audit record, never the authority), `container.json` (identity and the version read), and optionally `plan.ts` (a tracking plan against the recipe library) and `custom.ts` (bespoke entities), merged with `mergeSpecs` into the desired state. The repo config file is the one TASK-30 defines.
